...
export class AppointmentRequest extends FormRequest {
  ...
  public async save(){
    ...
    const appointment = this.route('appointment') ? await Appointment.find(this.route('appointment')) as AppointmentInterface : new Appointment() as AppointmentInterface;


     appointment.business_id = this.input('business_id');
